...
import { relatedLinksForDestination, airportsForStateCode, countryGuidePath, hotelPlaceLinksForCity } from "../src/data/seoLinkGraph";
...
indexableHotelDestinations.forEach((d) => {
  ...
  programmaticPages.push({
    ...
    links: [
      ...
      ...hotelPlaceLinksForCity(d.slug, 8),
...
// Individual hotel place pages (Google Places-backed, city must be indexable)
hotelPlaces
  .filter((h) => !!getHotelDestinationBySlug(h.citySlug))
  .forEach((h) => {
    const nearby = hotelPlaces
      .filter((p) => p.citySlug === h.citySlug && p.placeId !== h.placeId)
      .slice(0, 6);
    programmaticPages.push({
      path: hotelPlacePath(h),
      title: `${h.name}, ${h.cityName} — Rates & Availability | Tripile`.slice(0, 64),
      description: `${h.name} in ${h.cityName}${
        h.rating ? ` is rated ${h.rating}/5 by ${h.reviewCount?.toLocaleString()} Google reviewers` : ""
      }. See the location and compare live rates on Tripile.`.slice(0, 158),
      h1: h.name,
      blocks: [
        {
          paragraphs: [
            `${h.name} is located at ${h.address}, in ${h.cityName}. Enter your dates on Tripile to check live availability and compare current rates — prices are always pulled at search time.`,
            h.rating && h.reviewCount
              ? `Google reviewers rate ${h.name} ${h.rating}/5 from ${h.reviewCount.toLocaleString()} reviews${h.category ? `, listed as a ${h.category.toLowerCase()}` : ""}.`
              : `${h.name} is a ${h.category ?? "hotel"} in ${h.cityName} listed from verified Google Places data.`,
          ],
        },
        {
          heading: "Location",
          paragraphs: [
            `The full address is ${h.address}.${
      ...
      links: [
        { href: hotelDestinationPath(h.citySlug), label: `Hotels in ${h.cityName}` },
        ...nearby.map((p) => ({ href: hotelPlacePath(p), label: p.name })),
        { href: "/hotels", label: "Search all hotels" },
      ],
    });
  });

// Validated city-pair flight pages
flightRouteCatalog.forEach((r) => {
  const destSlug = r.destination.toLowerCase().replace(/\s+/g, "-");
  programmaticPages.push({
    path: `/flights/${r.slug}`,
    title: `Cheap Flights from ${r.origin} to ${r.destination} | Tripile`.slice(0, 65),
    description:
      `Compare cheap flights from ${r.origin} (${r.originCode}) to ${r.destination} (${r.destinationCode}) on Tripile. Live fares from observed carriers including ${r.airlines.slice(0, 3).join(", ")}.`.slice(
        0,
        160,
      ),
    h1: `Cheap Flights from ${r.origin} to ${r.destination}`,
    blocks: [
      {
        paragraphs: [
          `Search live flights from ${r.origin} (${r.originCode}) to ${r.destination} (${r.destinationCode}). This city pair returned bookable Duffel offers when validated, so the page exists only for a real, flyable route.`,
          r.minDurationMinutes
            ? `The fastest observed one-way time is about ${Math.floor(r.minDurationMinutes / 60)}h ${r.minDurationMinutes % 60}m. Carriers seen on the route include ${r.airlines.join(", ")}.`
            : `Carriers observed on this route include ${r.airlines.join(", ")}.`,
        ],
      },
    ],
    links: [
      { href: `/flights-to/${destSlug}`, label: `All flights to ${r.destination}` },
      { href: "/flights", label: "Search all flights" },
      { href: "/deals", label: "Latest flight deals" },
    ],
  });
