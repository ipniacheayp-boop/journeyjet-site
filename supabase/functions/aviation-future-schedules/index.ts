...
serve(async (req) => {
  ...
  try {
    ...
    const { iataCode, type, date } = await req.json();
    ...
    const url = `https://aviation-edge.com/v2/public/flightsFuture?${params.toString()}`;
    console.log("Fetching future schedules:", url.replace(apiKey, "***"));
    ...
    console.error("Future schedules error:", error);
