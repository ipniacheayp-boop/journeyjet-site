...
const corsHeaders = {
  "Access-Control-Allow-Origin": "https://tripile.com",
...
serve(async (req) => {
  ...
  try {
    ...
    let query = supabase.from("site_reviews").select("*", { count: "exact" }).eq("is_deleted", false);
    ...
    let allReviewsQuery = supabase.from("site_reviews").select("rating").eq("is_deleted", false);
