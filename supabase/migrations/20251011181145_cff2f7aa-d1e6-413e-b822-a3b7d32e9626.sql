-- Phase 1: Agent & Booking System Schema (Fixed)
...
DROP POLICY IF EXISTS "Admins can update any agent profile" ON public.agent_profiles;
DROP POLICY IF EXISTS "Agents can view their own wallet" ON public.agent_wallet;
DROP POLICY IF EXISTS "Admins can view all wallets" ON public.agent_wallet;
...
DROP POLICY IF EXISTS "Admins can update commissions" ON public.agent_commissions;
...
DROP TRIGGER IF EXISTS update_agent_wallet_updated_at ON public.agent_wallet;
