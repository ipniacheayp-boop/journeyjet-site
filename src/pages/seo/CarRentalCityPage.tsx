...
const CarRentalCityPage = () => {
  ...
  const city = resolveCarRentalCity(slug);
  
  if (!city) {
    return (
      <>
        <Helmet>
          <title>Car Rental City Not Found | Tripile</title>
          <meta name="robots" content="noindex, follow" />
        </Helmet>
        <Header />
        <div className="container mx-auto px-4 py-20 text-center">
          <h1 className="text-2xl font-bold mb-4">City Not Found</h1>
          <Link to="/car-rentals" className="text-primary hover:underline">Browse car rentals</Link>
        </div>
        <Footer />
      </>
    );
  ...
  const pageDescription = `Find affordable car rentals in ${city.city}${city.state ? `, ${city.state}` : ""}. Compare top providers and book instantly. Rentals starting at $${city.avgPrice}/day.`;
  const canonicalUrl = `https://tripile.com/cheap-car-rentals-in-${city.slug}`;
  ...
  const breadcrumbs = [
    ...
    { name: 'Car Rentals', url: 'https://tripile.com/deals' },
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto text-center">
              ...
              <p className="text-lg text-muted-foreground mb-8 max-w-2xl mx-auto">
                Compare car rental prices from top providers in {city.city}{city.state ? `, ${city.state}` : ""}. 
