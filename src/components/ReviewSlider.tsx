  ...
  return (
    <div className="w-full max-w-[700px] h-[170px] md:h-[160px] flex items-center justify-between bg-white/5 border border-white/10 rounded-xl px-2 md:px-4">
      ...
      <button
        ...
        className="w-8 h-8 md:w-9 md:h-9 flex items-center justify-center rounded-full border border-white/40 text-white text-xl hover:bg-white/20 transition shrink-0"
      >
        ‹
      </button>

      {/* Review */}
      <div className="flex-1 px-2 md:px-4 text-center text-white overflow-hidden flex flex-col justify-center h-full">
        {/* Stars */}
        <div className="flex justify-center mb-2">
          {Array.from({ length: 5 }).map((_, i) => (
            <span
              key={i}
              className={`text-base md:text-lg ${i < reviews[index].rating ? "text-green-400" : "text-white/30"}`}
            >
              ★
            </span>
          ))}
        ...
        <p className="text-sm md:text-base leading-relaxed line-clamp-4 md:line-clamp-3">“{reviews[index].message}”</p>

        <p className="mt-3 text-[10px] md:text-xs text-white/70">
