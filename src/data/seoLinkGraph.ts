...
import { hotelPlacesForCity, hotelPlacePath } from "./hotelPlaceCatalog";
...
/** Real Places-backed hotel pages for a city, when the catalog has them. */
export function hotelPlaceLinksForCity(citySlug: string, limit = 8): SeoLink[] {
  return hotelPlacesForCity(citySlug)
    .slice(0, limit)
    .map((h) => ({ href: hotelPlacePath(h), label: h.name }));
}
