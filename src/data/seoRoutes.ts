...
export interface ResolvedCarRentalCity {
  slug: string;
  city: string;
  state: string;
  avgPrice: number;
...
/** Resolve `/cheap-car-rentals-in-:slug` against popular destinations + editorial extras. */
export function resolveCarRentalCity(routeSlug?: string): ResolvedCarRentalCity | undefined {
  if (!routeSlug) return undefined;
  const slug = routeSlug.replace(/^cheap-car-rentals-in-/, "");
  const dest = popularDestinations.find((d) => d.slug === slug);
  const seo = seoCarCities.find(
    (c) => c.slug === routeSlug || c.slug === `cheap-car-rentals-in-${slug}`,
  );
  if (!dest && !seo) return undefined;
  return {
    slug: dest?.slug ?? slug,
    city: dest?.city ?? seo!.city,
    state: seo?.state ?? dest?.country ?? "",
    avgPrice: seo?.avgPrice ?? 32,
    topProviders: seo?.topProviders ?? ["Enterprise", "Hertz", "Budget", "Avis"],
  };
}
